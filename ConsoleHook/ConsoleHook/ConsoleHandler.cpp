...
, m_dwOldReadAreaStartIndex(0)
, m_dwOldNewDataEndIndex(0)
, m_dwOldCursorIndex(0)
{
}
...
bool ConsoleHandler::OpenSharedObjects()
{
	...
	//MessageBox(NULL, (SharedMemNames::formatInfo % dwProcessId).str().c_str(), L"ConsoleHook", MB_OK);
	...
	// console buffer info
	m_consoleBufferInfo.Open((SharedMemNames::formatBufferInfo % dwProcessId).str(), syncObjBoth);

	// console buffer
	m_consoleBuffer.Open((SharedMemNames::formatBuffer % dwProcessId).str(), syncObjNone);
...
//////////////////////////////////////////////////////////////////////////////
	...
	SHORT	nReadAreaRight	= csbiConsole.dwSize.X - 1;

	SHORT	nReadAreaWidth	= nReadAreaRight - nReadAreaLeft + 1;
	SHORT	nReadAreaHeight	= nReadAreaBottom - nReadAreaTop + 1;
...
//	TRACE(L"===================================================================\n");
	...
	SHORT nBufferArea	= coordBufferSize.X * coordBufferSize.Y;

	// initialize reading rectangle
	srBuffer.Top		= nReadAreaTop;
	srBuffer.Bottom		= min(nReadAreaTop + coordBufferSize.Y - 1, nReadAreaBottom);
	srBuffer.Left		= nReadAreaLeft;
	srBuffer.Right		= nReadAreaRight;
...
//		TRACE(L"Reading region: (%i, %i) - (%i, %i)\n", srBuffer.Left, srBuffer.Top, srBuffer.Right, srBuffer.Bottom);
		...
		srBuffer.Top	+= coordBufferSize.Y;
		srBuffer.Bottom	+= coordBufferSize.Y;

		dwScreenBufferOffset += nBufferArea;
	}

	// read the last 'chunk', we need to calculate the number of rows in the
	// last chunk and update bottom coordinate for the region
	coordBufferSize.Y	= nReadAreaBottom - srBuffer.Top + 1;
	srBuffer.Bottom		= nReadAreaBottom;

//////////////////////////////////////////////////////////////////////////////
// Old code for reading console buffer.
// -only reads buffer from current window rectangle.
//////////////////////////////////////////////////////////////////////////////
...
//	DWORD					dwScreenBufferOffset= 0;
//
//	MessageBox(NULL, (SharedMemNames::formatInfo % dwScreenBufferSize).str().c_str(), L"ConsoleHook", MB_OK);
//	shared_array<CHAR_INFO> pScreenBuffer(new CHAR_INFO[dwScreenBufferSize]);
//	MessageBox(NULL, (SharedMemNames::formatInfo % dwScreenBufferSize).str().c_str(), L"ConsoleHook", MB_OK);
//
//	COORD		coordBufferSize;
...
//	TRACE(L"===================================================================\n");
	...
	if (m_dwOldScreenBufferSize == 0)
	{
		...
		// fill old buffer with spaces
		for(DWORD i = 0; i < m_dwOldScreenBufferSize; i++)
			m_pOldScreenBuffer[i].Char.UnicodeChar = ' ';
	}
	...
	DWORD	dwBufIndex = dwCmpStartIndex - dwReadAreaStartIndex;
	DWORD	dwOldBufIndex = dwCmpStartIndex - m_dwOldReadAreaStartIndex;
	...
	if (bNewDataFound
		|| (::memcmp(m_consoleInfo.Get(), &csbiConsole, sizeof(CONSOLE_SCREEN_BUFFER_INFO)) != 0))
	//if (m_dwOldScreenBufferSize != dwScreenBufferSize) ||
	//   (::memcmp(m_consoleBuffer.Get(), pScreenBuffer.get(), m_dwOldScreenBufferSize*sizeof(CHAR_INFO)) != 0))
	{
		SharedMemoryLock bufferLock(m_consoleBuffer);

		// copy console info and cursor info to shared memory
...
//////////////////////////////////////////////////////////////////////////////
	...
	//		% bufferCmpIndex).str().c_str()), L"ConsoleHook", MB_OK);*/
	...
	//	// copy buffer and buffer info to shared memory.
	//	::CopyMemory(m_consoleBuffer.Get(), pScreenBuffer.get(), m_dwOldScreenBufferSize*sizeof(CHAR_INFO));
	//	::CopyMemory(m_consoleInfo.Get(), &csbiConsole, sizeof(CONSOLE_SCREEN_BUFFER_INFO));
	//	m_consoleBufferInfo->dwBufferSize = dwScreenBufferSize;
...
*/
		...
		switch (dwWaitRes)
		{
			...
			case WAIT_OBJECT_0 + 2 :
			{
				...
				MessageBox(NULL, reinterpret_cast<wchar_t*>(*m_consolePasteInfo.Get()), 
					L"ConsoleHook", MB_OK);*/
